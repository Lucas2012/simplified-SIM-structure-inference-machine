...
class structured_gate(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        # gate design:
        if self.on_edge:
            s_gate = numpy.zeros(bottom[0].shape[0])
        else:
            s_gate = numpy.zeros([bottom[0].shape[0],self.nAction])
    ...
    def backward(self, top, propagate_down, bottom):
        ...
        for i in range(0,self.bottom_batchsize):
            ...
            if self.regularizer == 1:
                gates_diff[i] = numpy.sum(diff)*(1-tanh_sq) + self.lamda*(1-tanh_sq)*self.C/2.0
            elif self.regularizer == 2:
                gates_diff[i] = numpy.sum(diff)*(1-tanh_sq) + self.lamda*s_gate[i]*(1-tanh_sq)*self.C
