...
class structured_gate(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        for f in range(0,self.bottom_batchsize):
            for i in range(0,self.nPeople):
                o = s_gate[idx]
                start_i = 75+(i+1)*self.message_num_action*self.nAction-self.nAction
    ...
    def backward(self, top, propagate_down, bottom):
        ...
        for f in range(0,self.bottom_batchsize):
            for i in range(0,self.nPeople):
                ...
                start_i = 75+(i+1)*self.message_num_action*self.nAction-self.nAction
                end_i = 75+(i+1)*self.message_num_action*self.nAction 
                diff_i = gated_message_diff[f,start_i:end_i]
                message_i = messages[f,start_i:end_i]
                diff_i = numpy.multiply(diff_i,message_i)
                ...
                if zero2one:
                    ...
                    divide = 1.0
                if on_edge == True:
                    gates_diff[count] = (1-tanh_sq)*numpy.sum(diff_i)/divide
                else:
                    gates_diff[count] = numpy.multiply((1-tanh_sq),diff_i)/divide
                count += 1
