...
class sum_scene(caffe.Layer):
    ...
    def reshape(self, bottom, top):
        ...
        self.top_output_num = self.nScene
    ...
    def forward(self, bottom, top):
        ...
        tmpdata = numpy.reshape(bottom[0].data,[self.top_batchsize,self.bottom_output_num/self.nScene,self.nScene])
        #print 'sum scene:'
        for i in range(0,self.top_batchsize):
            tmp = tmpdata[i].sum(axis = 0)
            top[0].data[i] = tmp/(self.label_stop[i]+1)
            #top[0].data[i] = tmpdata[i,self.nPeople]
        #print tmpdata[0,self.nPeople]
    ...
    def backward(self, top, propagate_down, bottom):
        ...
        for i in range(0,self.top_batchsize):
            ...
            fortmp = numpy.repeat(tmpdiff[i],self.bottom_output_num/self.nScene)
            fortmp = numpy.reshape(fortmp,self.bottom_output_num)
            tmp = fortmp/(label_stop[i]+1)
            bottom[0].diff[i] = tmp
