...
class Data_Arrange_Layer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        if self.ifaverage:
            bottom_data = bottom[0].data[:self.bottom_batchsize]
            for i in range(1,self.T_):
                bottom_data += bottom[0].data[i*self.bottom_batchsize:(i+1)*self.bottom_batchsize] 
            top[0].data[...] = bottom_data/self.T_
        else:
            top[0].data[...] = bottom[0].data[(self.T_-1)*self.bottom_batchsize:self.T_*self.bottom_batchsize]      
