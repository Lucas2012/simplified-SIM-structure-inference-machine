...
class simplified_pose_out(caffe.Layer):
    ...
    def reshape(self, bottom, top):
        ...
        self.bottom_batchsize = bottom[1].data.shape[0]/self.nPeople   #-->numbers of frame in a batch
    ...
    def forward(self, bottom, top):
        ...
        for f in range(0,self.bottom_batchsize):
            pose_f = bottom[0].data[step:step+self.label_stop[f]]
            top[0].data[f] = pose_f.sum(axis = 0)/(1.0*self.label_stop[f])
    ...
    def backward(self, top, propagate_down, bottom):
        ...
        for f in range(0,self.bottom_batchsize):
            ...
            for i in range(0,int(self.label_stop[f])):
                bottom[0].diff[step+i] = diff_f/(1.0*self.label_stop[f])
            step += self.label_stop[f]
