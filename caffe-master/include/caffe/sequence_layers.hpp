...
template <typename Dtype> class BP_RecurrentLayer;
 ...
 */
template <typename Dtype>
class BP_RecurrentLayer : public Layer<Dtype> {
 public:
  explicit BP_RecurrentLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "BP_Recurrent"; }
  virtual inline int MinBottomBlobs() const { return 2; }
 ...
 protected:
   ...
   *        should define this -- see RNNLayer and LSTMLayer for examples.
   */
  virtual void BP_FillUnrolledNet(NetParameter* net_param) const = 0;
   ...
   *        for examples.
   */
  virtual void BP_RecurrentInputBlobNames(vector<string>* names) const = 0;
   ...
   *        for examples.
   */
  virtual void BP_RecurrentOutputBlobNames(vector<string>* names) const = 0;
   ...
   *        examples.
   */
  virtual void BP_OutputBlobNames(vector<string>* names) const = 0;
  ...
  Blob<Dtype>* x_static_input_blob_;
  Blob<Dtype>* cont_input_blob_;
  Blob<Dtype>* MessageIn0_blob_;
  Blob<Dtype>* UnaryInput_blob_;
  Blob<Dtype>* label_blob_;
  Blob<Dtype>* fc8_context_;
  Blob<Dtype>* concat_all_;
};

/*
  simple trial for adding new recurrent layer --- by zhiwei
*/
template <typename Dtype>
class BP_RNNLayer : public BP_RecurrentLayer<Dtype> {
 public:
  explicit BP_RNNLayer(const LayerParameter& param)
      : BP_RecurrentLayer<Dtype>(param) {}

  virtual inline const char* type() const { return "BP_RNN"; }
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    // Can't propagate to labels.
    return bottom_index != 2;
  }

 protected:
  virtual void BP_FillUnrolledNet(NetParameter* net_param) const;
  virtual void BP_RecurrentInputBlobNames(vector<string>* names) const;
  virtual void BP_RecurrentOutputBlobNames(vector<string>* names) const;
  virtual void BP_OutputBlobNames(vector<string>* names) const;
};
