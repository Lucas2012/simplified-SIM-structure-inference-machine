...
class Message_Reshape3(caffe.Layer):
    ...
    def reshape(self, bottom, top):
        ...
        self.botshape = bottom_shape
        top[0].reshape(bottom_batchsize, bottom_shape[1]*bottom_shape[2]*bottom_shape[3])
        self.bottom_batchsize = bottom_batchsize
        self.nPeople = bottom_shape[1]
        self.unitlen = bottom_shape[2]
        self.output_num = self.botshape[1]*self.botshape[2]*self.botshape[3]

    def forward(self, bottom, top):
        top[0].data[...] = numpy.reshape(bottom[0].data,[self.bottom_batchsize,self.output_num])

    def backward(self, top, propagate_down, bottom):
        bottom[0].diff[...] = numpy.reshape(top[0].diff,[self.botshape])
